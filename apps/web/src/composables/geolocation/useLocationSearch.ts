...
import { ENUM_JSDOC_FILTERED_FUZZY_TYPES } from '~/types/locationSearch/enum'
 ...
 */
export function useLocationSearch(searchState: Ref<string>, choosenLocation: Ref<LocationCoordinates | null>) {
    const queryResults = ref<ProcessedLocation[]>([])
...
function createGeoBias(
    ...
    choosenLocation: LocationCoordinates | null,
...
async function fetchFuzzyResults(
    ...
    geobias: string,
): Promise<ProcessedLocation[] | null> {
    const runtimeConfig = useRuntimeConfig()
    const tomtomConfig = runtimeConfig.public.tomtom
